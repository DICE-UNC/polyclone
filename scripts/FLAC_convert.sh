...
if [ "$extI" = "$extO" ]                    || 
   ...
   [ "$extI" = "aif" -a "$extO" = "aiff" ]   ; then
    ...
    /usr/bin/flac -d "$1" -f -o "$2"
elif [  "$extO" = "flac" ] || [ "$extO" = "fla" ]; then
    /usr/bin/flac --best "$1" -f -o "$2"
else  # this is to convert wav <-falc-> aiff or aiff <-flac-> wav  
    mkdir /tmp/FlacConversion_$randomDir
    /usr/bin/flac --best "$1" -f -o "/tmp/FlacConversion_$randomDir/temp.flac"
    /usr/bin/flac -d "/tmp/FlacConversion_$randomDir/temp.flac" -f -o "$2"
